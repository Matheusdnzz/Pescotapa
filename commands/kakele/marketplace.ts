...
export default new InteractionCommand({
  data: new SlashCommandBuilder()
    .setName("marketplace")
    .setDescription(
      "External storefront for the game Kakele, to display and purchase items.",
    )
    .setDescriptionLocalizations({
      "es-ES":
        "Tienda externa para el juego Kakele, para mostrar y comprar objetos.",
  ...
  async run(
    ...
    async function manageMarketplace(): Promise<void> {
      await interaction
        .editReply({
          components: [
            new ActionRowBuilder<ButtonBuilder>()
              .addComponents(
                MARKETPLACE_MAIN_ADD(
                  client.translate("MARKETPLACE_MAIN_ADD", args.language),
                ),
              )
              .addComponents(
                MARKETPLACE_MAIN_MANAGE(
                  client.translate("MARKETPLACE_MAIN_MANAGE", args.language),
                ),
              )
              .addComponents(
                MARKETPLACE_MAIN_RULES(
                  client.translate("MARKETPLACE_MAIN_RULES", args.language),
                ),
              )
              .addComponents(
                MARKETPLACE_MAIN_EXIT(
                  client.translate("MARKETPLACE_MAIN_EXIT", args.language),
                ),
              ),
          ],
          files: [mainMenuImage],
        ...
        .then((message) => {
          let item: { name: string; emoji: string; raw: any } = {
            name: "` `",
            emoji: "` `",
            raw: [],
          };
          ...
          function getPriceString(): string {
            return `${
              ...
              price.money.amount > 0
                ? `\`${new Intl.NumberFormat().format(price.money.amount)} ${
          ...
          async function modalErr(err: Error): Promise<void> {
            if (err.name === "Error [InteractionCollectorError]") {
              await interaction.followUp({
                content: client.translate(
                  "MARKETPLACE_ERROR_MODAL_RESPONSE_TITLE",
                  args.language,
                ),
                ephemeral: true,
              });
            ...
            logger.error("modalErrCb", err);
          }

          async function createItemOffer(
            ...
            await MarketplaceItem.create({
              ...
              .then(async (res: IOfferItemDocument) => {
                logger.audit(
                  `Item added by ${
                    interaction.user.id
                  } with ObjectId: ${res._id.toHexString()}`,
                  `${item.name}`,
              ...
              .catch((err) => {
                logger.error("showSetGenericPriceModal", err);
              });
          }

          async function sendCreatedOfferMessage(
            i: MessageComponentInteraction,
            id: Types.ObjectId,
          ): Promise<void> {
            ...
            const string = `- ${item.emoji} ${amount}x \`${
              item.name
            }\` ${getStatsString()} ${getPriceString()}`;
            ...
            await i.followUp({
              embeds: [
                new CustomEmbed()
                  .setDescription(
                    client.translate(
                      "MARKETPLACE_ADD_CONFIRMATION_TITLE",
                      args.language,
                      { item: string },
                    ),
                  )
            ...
            await i.editReply({
              embeds: [
                new CustomEmbed()
                  .setTitle(
                    client.translate(
                      "MARKETPLACE_ADD_EMBED_TITLE",
                      args.language,
                    ),
                  )
                  .setDescription(
                    client.translate(
                      "MARKETPLACE_ADD_EMBED_DESCRIPTION",
                      args.language,
                      {
                        item: `${amount > 0 ? `${amount}x` : ""} ${
                          item.emoji
                        } ${item.name}`,
                  ...
                  .setFooter({
                    text: client.translate(
                      "MARKETPLACE_ADD_EMBED_FOOTER_MAIN",
                      args.language,
                    ),
                  }),
              ],
              components: [
                new ActionRowBuilder<ButtonBuilder>()
                  .addComponents(
                    MARKETPLACE_ADD_BACK(
                      client.translate("MARKETPLACE_ADD_BACK", args.language),
                    ),
                  )
                  .addComponents(
                    MARKETPLACE_ADD_ITEM(
                  ...
                  .addComponents(
                    MARKETPLACE_ADD_STATS(
                      client.translate("MARKETPLACE_ADD_STATS", args.language),
                    ).setDisabled(
                      item.name === "` `" || !("stats" in item.raw),
                    ),
                  )
                  .addComponents(
                    MARKETPLACE_ADD_SERVER(
                      client.translate("MARKETPLACE_ADD_SERVER", args.language),
                    ).setDisabled(item.name === "` `"),
                  )
                  .addComponents(
                    MARKETPLACE_ADD_PRICE(
                      client.translate("MARKETPLACE_ADD_PRICE", args.language),
                    ).setDisabled(item.name === "` `" || servers.length === 0),
                  ),
              ],
              files: [],
            });
          ...
          async function showItemResultSelectionRow(
            items: any[],
            i: MessageComponentInteraction,
          ): Promise<void> {
            if (!i.deferred && !i.replied) {
            ...
            await i.editReply({
              embeds: [
                new CustomEmbed()
                  ...
                  .setFooter({
                    text: client.translate(
                      "MARKETPLACE_ADD_EMBED_FOOTER_ITEM_SELECTION",
                      args.language,
                    ),
                  }),
              ],
              components: [
                ...
                new ActionRowBuilder<ButtonBuilder>().addComponents(
                  MARKETPLACE_ADD_BACK(
                    client.translate("MARKETPLACE_ADD_BACK", args.language),
                  ).setCustomId("MARKETPLACE_ADD_BACK_GENERIC"),
                ),
              ],
            });
          }

          async function showSetCurrencyRow(
            i: MessageComponentInteraction,
          ): Promise<void> {
            if (!i.deferred && !i.replied) {
              await i.deferUpdate();
            ...
            await i.editReply({
              embeds: [
                new CustomEmbed()
                  ...
                  .setFooter({
                    text: client.translate(
                      "MARKETPLACE_ADD_EMBED_FOOTER_CURRENCY_SELECTION",
                      args.language,
                    ),
                  }),
              ],
              components: [
                ...
                new ActionRowBuilder<ButtonBuilder>()
                  .addComponents(
                    MARKETPLACE_ADD_BACK(
                      client.translate("MARKETPLACE_ADD_BACK", args.language),
                    ).setCustomId("MARKETPLACE_ADD_BACK_CURRENCY_SELECTION"),
                  )
                  .addComponents(
                    MARKETPLACE_ADD_VALUE_MONEY_VALUE(
                      client.translate(
          ...
          async function showItemPriceRow(
            i: MessageComponentInteraction,
          ): Promise<void> {
            if (!i.deferred && !i.replied) {
              await i.deferUpdate();
            ...
            await i.editReply({
              embeds: [
                new CustomEmbed()
                  ...
                  .setFooter({
                    text: client.translate(
                      "MARKETPLACE_ADD_EMBED_FOOTER_PRICE",
                      args.language,
                    ),
                  }),
              ],
              components: [
                new ActionRowBuilder<ButtonBuilder>()
                  .addComponents(
                    MARKETPLACE_ADD_BACK(
                      client.translate("MARKETPLACE_ADD_BACK", args.language),
                    ).setCustomId("MARKETPLACE_ADD_PRICE_BACK"),
                  )
                  .addComponents(
                    MARKETPLACE_ADD_PRICE_COINS(
                      client.translate(
          ...
          async function showItemSetStatsRow(
            i: MessageComponentInteraction,
          ): Promise<void> {
            if (!i.deferred && !i.replied) {
              await i.deferUpdate();
            ...
            await i.editReply({
              embeds: [
                new CustomEmbed()
                  ...
                  .setFooter({
                    text: client.translate(
                      "MARKETPLACE_ADD_EMBED_FOOTER_STATS",
                      args.language,
                    ),
                  }),
              ],
          ...
          async function showServerSelectionRow(
            i: MessageComponentInteraction,
          ): Promise<void> {
            if (!i.deferred && !i.replied) {
              await i.deferUpdate();
            ...
            await i.editReply({
              embeds: [
                new CustomEmbed()
                  ...
                  .setFooter({
                    text: client.translate(
                      "MARKETPLACE_ADD_EMBED_FOOTER_GENERIC_SERVER_SELECTION",
                      args.language,
                    ),
                  }),
              ],
          ...
          async function showSetCurrencyValueModal(
            i: MessageComponentInteraction,
          ): Promise<void> {
            await i.showModal(MARKETPLACE_ADD_PRICE_MODAL(client, args));

            await i
              .awaitModalSubmit({ time: 30 * 1000 })
              .then(async (modalResponse) => {
                await modalResponse.deferUpdate();

                const inputAmount = parseInt(
                  (
                    modalResponse.components[0]
                      .components[0] as TextInputModalData
                  ).value,
                );
                ...
                if (Number.isNaN(inputAmount) || inputAmount <= 0) {
                  await i.followUp({
                    content: client.translate(
                      "MARKETPLACE_GENERIC_PRICE_VALUE_TITLE",
                      args.language,
                    ),
                    ephemeral: true,
                  });
          ...
          async function showItemSelectionModal(
            ...
            await i
              ...
              .then(async (modalResponse) => {
                ...
                const inputItem = modalResponse.components[0]
                  .components[0] as TextInputModalData;
                const inputAmount = parseInt(
                  (
                    modalResponse.components[1]
                      .components[0] as TextInputModalData
                  ).value,
                );
                ...
                if (result.length > 0) {
                  if (result.length === 1) {
                    const emoji = client.kakeleEmojis.find(
                      (x) =>
                        x.name ===
                        result[0].item.name.english.replace(/[^A-Z0-9]/gi, ""),
                    );
                  ...
                  await i.followUp({
                    content: client.translate(
                      "MARKETPLACE_UNKNOWN_RESULT_TITLE",
                      args.language,
                    ),
                    ephemeral: true,
                  });
          ...
          async function showSetStatsModal(
            i: MessageComponentInteraction,
          ): Promise<void> {
            ...
            await i
              ...
              .then(async (modalResponse) => {
                ...
                const statType = i.customId
                  .replace("MARKETPLACE_ADD_STATS_", "")
                  .toLocaleLowerCase();
                ...
                async function handleStat(
                  ...
                  if (Number.isNaN(amount) || amount > max) {
                    await i.followUp({
                      content: client.translate(
                        "MARKETPLACE_UNKNOWN_STAT_VALUE_TITLE",
                        args.language,
                      ),
                      ephemeral: true,
                    });
                ...
                switch (statType) {
                  ...
                  default:
                    await i.followUp({
                      content: client.translate(
                        "MARKETPLACE_UNKNOWN_STAT_TYPE_TITLE",
                        args.language,
                      ),
                      ephemeral: true,
                    });
                    throw new Error(
                      "showSetStatsModal - Invalid item stat type",
                    );
                }
              })
              .catch((err) => {
          ...
          async function showSetGenericPriceModal(
            i: MessageComponentInteraction,
          ): Promise<void> {
            await i.showModal(MARKETPLACE_ADD_PRICE_MODAL(client, args));
            ...
            await i
              ...
              .then(async (modalResponse) => {
                ...
                const priceType = i.customId
                  .replace("MARKETPLACE_ADD_PRICE_", "")
                  .toLocaleLowerCase();
                ...
                if (Number.isNaN(inputAmount) || inputAmount <= 0) {
                  await i.followUp({
                    content: client.translate(
                      "MARKETPLACE_UNKNOWN_GENERIC_PRICE_VALUE_TITLE",
                      args.language,
                    ),
                    ephemeral: true,
                  });
          ...
          async function handleResultSelection(
            ...
            if (selectedOption?.emoji !== undefined) {
              item.emoji = `<:${selectedOption.emoji?.name as string}:${
                selectedOption.emoji?.id as string
              }>`;
            }

            item.raw = client.kakeleItems.find(
              (x) => x.name[language] === value,
            );
          ...
          async function handleBackPriceButton(
            i: MessageComponentInteraction,
          ): Promise<void> {
            if (!i.deferred && !i.replied) {
              await i.deferUpdate();
          ...
          interactionCollector.on(
            "collect",
            async (
              interactionResponse: MessageComponentInteraction<"cached">,
            ): Promise<any> => {
              if (interactionResponse.user.id !== interaction.user.id) {
                ...
                return await interactionResponse.followUp({
                  content: client.translate(
                    "MARKETPLACE_COLLECTOR_NOT_OWNER_TITLE",
                    args.language,
                  ),
                  ephemeral: true,
                });
              ...
              switch (interactionResponse.customId) {
                ...
                case "MARKETPLACE_ADD_RESULT_SELECTION_MENU":
                  await handleResultSelection(
                    interactionResponse as StringSelectMenuInteraction,
                  );
                  break;
    ...
    async function viewMarketplace(): Promise<void> {
        ...
        await interaction.editReply({
          embeds: [
            new CustomEmbed()
              .setTitle(
                client.translate(
                  "MARKETPLACE_VIEW_UNKNOWN_DATA_TITLE",
                  args.language,
                ),
              )
              .setDescription(
                client.translate(
                  "MARKETPLACE_VIEW_UNKNOWN_DATA_DESCRIPTION",
                  args.language,
                ),
              )
              .setAuthor({
      ...
      const pageEntries: IOfferItemDocument[][] = chunkArray(
        entries.sort((a, b) => {
      ...
      let image: Canvas = await createItemListImage(
        pageEntries[pageIndex],
        `${pageIndex + 1}/${pageEntries.length}`,
      );
        ...
        .editReply({
          ...
          components: [
            new ActionRowBuilder<ButtonBuilder>()
              .addComponents(
                MARKETPLACE_VIEW_EXIT(
                  client.translate("MARKETPLACE_VIEW_EXIT", args.language),
                ),
              )
              .addComponents(MARKETPLACE_VIEW_HOME_PAGE())
              .addComponents(
                MARKETPLACE_VIEW_PREVIOUS_PAGE().setDisabled(
                  pageEntries[pageIndex - 1] === undefined,
                ),
              )
              .addComponents(
                MARKETPLACE_VIEW_NEXT_PAGE().setDisabled(
                  pageEntries[pageIndex + 1] === undefined,
                ),
              ),
            new ActionRowBuilder<ButtonBuilder>()
              .addComponents(
                MARKETPLACE_VIEW_FILTER(
                  client.translate("MARKETPLACE_VIEW_FILTER", args.language),
                ),
              )
              .addComponents(
                MARKETPLACE_VIEW_SELECT(
                  client.translate("MARKETPLACE_VIEW_SELECT", args.language),
                ),
              )
              .addComponents(
                MARKETPLACE_VIEW_HELP(
                  client.translate("MARKETPLACE_VIEW_HELP", args.language),
                ),
              ),
          ],
        })
        .then((message) => {
          ...
          async function showNextPage(
            ...
            if (cachedPages[pageIndex] !== undefined) {
              ...
              image = await createItemListImage(
                pageEntries[pageIndex],
                `${pageIndex + 1}/${pageEntries.length}`,
              );
          ...
          async function showPreviosPage(
            i: MessageComponentInteraction,
          ): Promise<void> {
            pageIndex =
              pageEntries[pageIndex - 1] === undefined
          ...
          async function showMainMenuRow(
            i: MessageComponentInteraction,
            reloadImage: boolean,
          ): Promise<void> {
            if (!i.deferred && !i.replied) {
          ...
          async function showItemResultSelectionRow(
            ...
            await interaction.editReply({
              components: [
                new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
                  MARKETPLACE_VIEW_SELECTION_MENU(pageEntries[pageIndex]),
                ),
                new ActionRowBuilder<ButtonBuilder>().addComponents(
                  MARKETPLACE_VIEW_BACK(
                    client.translate("MARKETPLACE_VIEW_BACK", args.language),
                  ),
                ),
              ],
            });
          ...
          async function showSelectedItemRow(
            i: StringSelectMenuInteraction,
          ): Promise<void> {
            if (!i.deferred && !i.replied) {
              await i.deferUpdate();
            ...
            const item = entries.find(
              (x) => x._id.toHexString() === i.values[0],
            );
              ...
              await i.followUp({
                embeds: [
                  new CustomEmbed()
                    .setTitle(
                      client.translate(
                        "MARKETPLACE_VIEW_UNKNOWN_SELECTED_ITEM_TITLE",
                        args.language,
                      ),
                    )
                    .setDescription(
                      client.translate(
                        "MARKETPLACE_VIEW_UNKNOWN_SELECTED_ITEM_DESCRIPTION",
                        args.language,
                      ),
                    )
                    .setAuthor({
            ...
            //           .setAuthor({ name: 'Pescotapa', iconURL: client.icons.ElderVampireBrooch })
            //           .setColor(client.colors.DarkRed)
            //       ],
            //       ephemeral: true
            //     }
            ...
            const string = `- ${client.getKakeleItemEmoji(item?.item.name)} ${
              item.amount
            }x \`${item.item.name}\` ${getStatsString(item)} ${getPriceString(
              item,
            )}`;
            ...
            await i.followUp({
              embeds: [
                new CustomEmbed()
                  .setDescription(
                    client.translate(
                      "MARKETPLACE_VIEW_SELECTED_ITEM_TITLE",
                      args.language,
                      { item: string },
                    ),
                  )
