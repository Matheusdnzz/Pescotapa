...
async function createPlayerInfoImage(
  ...
  const dailyPlayerData: CharacterData = global.dailyPlayerData?.find(
  ...
  const texts: TextData[] = [
    // text
    {
      text: client.translate(`PLAYER_VOCATION_${player.vocation}`, language),
      x: 600,
      y: 115,
      fontSize: 40,
      font: "Vampire",
    ...
    // player data
    {
      text: String(getLevel(Number(player.experience))),
      x: 640 + 120,
      y: 265,
      fontSize: 25,
      font: "Helvetica",
    ...
    {
      text: new Intl.NumberFormat().format(Number(player.total_attack)),
      x: 640 + 120,
      y: 415,
      fontSize: 25,
      font: "Helvetica",
    ...
    {
      text: new Intl.NumberFormat().format(Number(player.total_magic)),
      x: 640 + 120,
      y: 455,
      fontSize: 25,
      font: "Helvetica",
    ...
    {
      text: new Intl.NumberFormat().format(Number(player.total_armor)),
      x: 640 + 120,
      y: 495,
      fontSize: 25,
      font: "Helvetica",
    ...
    {
      text: new Intl.NumberFormat().format(Number(player.gold)),
      x: 1000 + 270,
      y: 265,
      fontSize: 25,
      font: "Helvetica",
    ...
    {
      text: new Intl.NumberFormat().format(Number(player.achievements)),
      x: 1000 + 270,
      y: 305,
      fontSize: 25,
      font: "Helvetica",
    ...
    {
      text: capitalizeFirstLetter(player.server ?? ""),
      x: 1000 + 270,
      y: 345,
      fontSize: 25,
      font: "Helvetica",
    ...
    {
      text: new Intl.NumberFormat().format(Number(player.experience)),
      x: 1000 + 270,
      y: 415,
      fontSize: 25,
      font: "Helvetica",
    ...
    },
    {
      text: new Intl.NumberFormat().format(
        Number(dailyPlayerData?.progress ?? 0),
      ),
      x: 1000 + 270,
      y: 455,
      fontSize: 25,
    ...
    {
      text: new Intl.NumberFormat().format(
        getExperienceToNextLevel(getLevel(Number(player.experience))),
      ),
      x: 1000 + 270,
      y: 495,
      fontSize: 25,
  ...
  ctx.drawImage(
    await loadImage(
      `./src/assets/player/vocation/${player.vocation.toLowerCase()}/${random}.png`,
    ),
    90,
    0,
    512,
  ...
  if (
    fs.existsSync(
      `./src/assets/player/guilds/${player.guild}-${player.server ?? ""}.png`,
    )
  ) {
...
export default new InteractionCommand({
  ...
  async run(
    ...
    const result = fuse
      .search(playerInput)
      .filter(
        (e) => typeof e.score === "number" && e.score <= playerSearchThreshold,
      )
    ...
    if (result.length > 1) {
      ...
      const actionRow =
        new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
          PLAYER_RESULT_SELECTION_MENU(result),
        );
        ...
        .then((message) => {
          ...
          collector.on("collect", async (i) => {
            if (i.user.id === interaction.user.id) {
              ...
              const image = await createPlayerInfoImage(
                client,
                result[index].item,
                args.language,
              );
