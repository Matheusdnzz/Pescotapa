...
async function createExperienceImage(
  ...
  const canvas = createCanvas(
    cachedImages.experience.width,
    cachedImages.experience.height,
  );
  ...
  const experience =
    getExperienceToLevel(finalLevel) - getExperienceToLevel(initialLevel);
...
async function createNegativeExperienceImage(
  ...
  const canvas = createCanvas(
    cachedImages.negativeExperience.width,
    cachedImages.negativeExperience.height,
  );
  const ctx = canvas.getContext("2d");

  ctx.drawImage(
    cachedImages.negativeExperience,
    0,
    0,
    canvas.width,
    canvas.height,
...
export default new InteractionCommand({
  data: new SlashCommandBuilder()
    .setName("experience")
    .setDescription(
      "Calculate the amount of XP necessary to reach from one level to another.",
    )
    .setDescriptionLocalizations({
      "es-ES":
        "Calcula la cantidad de XP necesaria para llegar de un nivel inicial a otro nivel final.",
      "pt-BR":
        "Calcula a quantidade de XP necessária para chegar de um nível inicial a outro nível final.",
  ...
  async run(
      ...
      return await interaction.editReply({
        embeds: [
          new CustomEmbed()
            .setTitle(
              client.translate(
                "EXPERIENCE_NULL_INITIAL_FINAL_LEVEL",
                args.language,
              ),
            )
            .setDescription(
              client.translate(
                "EXPERIENCE_NULL_INITIAL_FINAL_LEVEL_DESCRIPTION",
                args.language,
              ),
            )
            .setAuthor({
      ...
      return await interaction.editReply({
        embeds: [
          new CustomEmbed()
            .setTitle(
              client.translate(
                "EXPERIENCE_EQUAL_INITIAL_FINAL_LEVEL",
                args.language,
              ),
            )
            .setDescription(
              client.translate(
                "EXPERIENCE_EQUAL_INITIAL_FINAL_LEVEL_DESCRIPTION",
                args.language,
              ),
            )
            .setAuthor({
    ...
    if (finalLevel < initialLevel) {
      image = await createNegativeExperienceImage(
        client,
        initialLevel,
        finalLevel,
        args.language,
      ...
      image = await createExperienceImage(
        client,
        initialLevel,
        finalLevel,
        args.language,
